...
local T1UIColor = {
	...
	["Logo Image"] = "rbxassetid://112485471724320",
	["Search Icon Color"] = Color3.fromRGB(240, 240, 230),
	["Search Icon Highlight Color"] = Color3.fromRGB(255, 80, 80),
	...
	["Title Text Color"] = Color3.fromRGB(255, 80, 80),
	...
	["Background Image"] = "",
	["Page Selected Color"] = Color3.fromRGB(255, 80, 80),
	["Section Text Color"] = Color3.fromRGB(220, 220, 210),
	["Section Underline Color"] = Color3.fromRGB(255, 80, 80),
	["Toggle Border Color"] = Color3.fromRGB(255, 80, 80),
	...
	["Dropdown Selected Color"] = Color3.fromRGB(255, 80, 80),
	["Dropdown Selected Check Color"] = Color3.fromRGB(219, 64, 64),
	["Textbox Highlight Color"] = Color3.fromRGB(255, 80, 80),
	["Box Highlight Color"] = Color3.fromRGB(255, 80, 80),
	["Slider Line Color"] = Color3.fromRGB(255, 80, 80),
...
local libCreateNoti = function(Setting)
    ...
    local Desc = Setting.Desc or Setting.Content or "";
    local Timeshow = Setting.ShowTime or Setting.Duration or 10;
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "255,80,80") .. ")\">" .. tostring(TitleNameMain or "Lonely Hub") .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
...
-- Thêm hàm table.find nếu chưa có
...
-- Đảm bảo UIColor được định nghĩa
if not UIColor then
    UIColor = getgenv().UIColor or T1UIColor
end
...
function Library:CreateWindow(Setting)
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = "<font color=\"rgb(" .. tostring(color or "255,80,80") .. ")\">" .. tostring(TitleNameMain or "Lonely Hub") .. "</font> " .. tostring(getgenv().MainDesc or "")
...
local function GlobalSearch(searchText)
	...
	function Main_Function:AddTab(PageName)
		...
		PageContainer.BackgroundColor3 = Color3.fromRGB(28, 28, 34)
		PageContainer.BackgroundTransparency = 0.25
		...
		pageContainerStroke.Color = Color3.fromRGB(90, 90, 70)
		pageContainerStroke.Thickness = 1

		local pageContainerGradient = Instance.new("UIGradient", PageContainer)
		pageContainerGradient.Color = ColorSequence.new{
			ColorSequenceKeypoint.new(0, Color3.fromRGB(28, 28, 34)),
			ColorSequenceKeypoint.new(1, Color3.fromRGB(38, 38, 46))
		}
		pageContainerGradient.Rotation = 90
		pageContainerGradient.Transparency = NumberSequence.new{
...
function sectionFunction:AddMultiDropdown(idk, Setting)
    ...
    DropdownFrame.Name = Title .. "MultiDropdownFrame"
    DropdownFrame.Parent = Section
    DropdownFrame.BackgroundColor3 = Color3.fromRGB(230, 230, 230)
    DropdownFrame.BackgroundTransparency = 1.000
    DropdownFrame.Position = UDim2.new(0, 0, 0.473684222, 0)
    ...
    local function SearchDropdown()
        local Results = {}
        for i, v in pairs(searchtable) do
            if string.find(v, string.lower(Dropdowntitle.Text)) then
                table.insert(found, v)
            end
        end
        for a, b in pairs(ScrollContainer:GetChildren()) do
            for c, d in pairs(found) do
                if d == string.lower(b.Name) then
                    b.Visible = true
                end
            end
        end
    ...
    local function refreshlist()
        ...
        if Selected then
            for i, v in pairs (ListNew) do
                ...
                SampleItemBG.Parent = SampleItem
                SampleItemBG.AnchorPoint = Vector2.new(0.5, 0.5)
                SampleItemBG.BackgroundColor3 = SelectedValues[i] and getgenv().UIColor["Dropdown Selected Check Color"] or Color3.fromRGB(255, 255, 255)
                SampleItemBG.BackgroundTransparency = SelectedValues[i] and 0.5 or 1
                SampleItemBG.BorderColor3 = Color3.fromRGB(27, 42, 53)
                SampleItemBG.Position = UDim2.new(0.5, 0, 0.5, 0)
                SampleItemBG.Size = UDim2.new(1, 0, 1, 0)
                ...
                SampleItemCheck.ImageRectOffset = Vector2.new(312, 4)
                SampleItemCheck.ImageRectSize = Vector2.new(24, 24)
                SampleItemCheck.ImageTransparency = SelectedValues[i] and 0 or 1
                ...
                SampleItemButton.MouseButton1Click:Connect(function()
                    SelectedValues[i] = not SelectedValues[i]
                    
                    TweenService:Create(
                        SampleItemCheck,
                        TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), {
                            ImageTransparency = SelectedValues[i] and 0 or 1
                        }
                    ):Play()
                    ...
                    TweenService:Create(
                        SampleItemBG,
                        TweenInfo.new(getgenv().UIColor["Tween Animation 1 Speed"]), {
                            BackgroundColor3 = SelectedValues[i] and getgenv().UIColor["Dropdown Selected Check Color"] or Color3.fromRGB(255, 255, 255),
                            BackgroundTransparency = SelectedValues[i] and 0.5 or 1
                        }
                    ):Play()
    ...
    local multiDropdownFunction = {}
    
    function multiDropdownFunction:SetValue(values)
        SelectedValues = {}
    ...
    function multiDropdownFunction:GetValue()
        local selectedArray = {}
        for val, isSel in pairs(SelectedValues) do
            if isSel then
                table.insert(selectedArray, val)
    ...
    function multiDropdownFunction:RefreshList(newList)
        ListNew = newList or ListNew
        SelectedValues = {}
        refreshlist()
        updateTitle()
    ...
    function multiDropdownFunction:ClearSelection()
        SelectedValues = {}
        refreshlist()
        updateTitle()
    ...
    function multiDropdownFunction:SelectAll()
        SelectedValues = {}
        for _, value in ipairs(ListNew) do
            SelectedValues[value] = true
        end
        refreshlist()
        updateTitle()
...
function sectionFunction:AddButton2(Setting)
    ...
    ButtonBG.Name = "ButtonBG"
    ButtonBG.Parent = TogFrame1
    ButtonBG.AnchorPoint = Vector2.new(0.5, 0.5)
    ButtonBG.Position = UDim2.new(0.5, 0, 0.5, 0)
    ButtonBG.Size = UDim2.new(1, 0, 1, 6)
    ButtonBG.BackgroundColor3 = getgenv().UIColor["Button Color"]
    ButtonBG.BackgroundTransparency = getgenv().UIColor["Background 1 Transparency"]
    ButtonBG.ZIndex = 0
    ...
    if Description and Description ~= "" then
        ...
        ButtonDescription.Parent = TogFrame1
        ButtonDescription.BackgroundColor3 = Color3.fromRGB(230, 230, 230)
        ButtonDescription.BackgroundTransparency = 1.000
        ButtonDescription.Position = UDim2.new(0, 15, 0, 20)
        ButtonDescription.Size = UDim2.new(1, -20, 0, 0)
        ButtonDescription.Font = Enum.Font.GothamBlack
        ButtonDescription.Text = Description
        ButtonDescription.TextSize = 13.000
        ButtonDescription.TextWrapped = true
        ButtonDescription.TextXAlignment = Enum.TextXAlignment.Left
        ButtonDescription.RichText = true
        ButtonDescription.AutomaticSize = Enum.AutomaticSize.Y
        ButtonDescription.TextColor3 = getgenv().UIColor["Toggle Desc Color"]
    end
    
    ButtonTitle.Name = "TextColor"
    ButtonTitle.Parent = TogFrame1
    ButtonTitle.BackgroundColor3 = Color3.fromRGB(230, 230, 230)
    ButtonTitle.BackgroundTransparency = 1.000
    ButtonTitle.Position = UDim2.new(0, 10, 0, cac)
    ButtonTitle.Size = UDim2.new(1, -10, 0, 20)
    ButtonTitle.Font = Enum.Font.GothamBlack
    ButtonTitle.Text = Title
    ButtonTitle.TextSize = 14.000
    ButtonTitle.TextXAlignment = Enum.TextXAlignment.Left
    ...
    ButtonTitle.RichText = true
    ButtonTitle.AutomaticSize = Enum.AutomaticSize.Y
    ButtonTitle.TextColor3 = getgenv().UIColor["Text Color"]
    ButtonTitle.ZIndex = 4
    
    -- Hover effect
    HoverFrame.Name = "HoverFrame"
    HoverFrame.Parent = TogFrame1
    HoverFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    HoverFrame.BackgroundTransparency = 0.9
    HoverFrame.Size = UDim2.new(1, 0, 1, 0)
    HoverFrame.ZIndex = 2
    ...
    -- Click effect (phóng từ tâm)
    ClickEffect.Name = "ClickEffect"
    ClickEffect.Parent = TogFrame1
    ClickEffect.AnchorPoint = Vector2.new(0.5, 0.5)
    ...
    Button.MouseEnter:Connect(function()
        ...
        TweenService:Create(TogFrame1, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(1, -5, 0, 0)
        }):Play()
        TweenService:Create(ButtonBG, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            BackgroundColor3 = getgenv().UIColor["Button Color"]:Lerp(Color3.new(1, 1, 1), 0.1)
        }):Play()
    end)
    ...
    Button.MouseButton1Click:Connect(function()
        ...
        -- Hiệu ứng nhấn xuống
        TweenService:Create(TogFrame1, TweenInfo.new(0.1, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(1, -15, 0, 0)
        }):Play()
        TweenService:Create(ButtonBG, TweenInfo.new(0.1), {
            BackgroundTransparency = getgenv().UIColor["Background 1 Transparency"] + 0.2
        }):Play()
        
        -- Click effect animation (phóng to từ tâm)
        TweenService:Create(ClickEffect, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Size = UDim2.new(1, 0, 1, 0),
            BackgroundTransparency = 1
        }):Play()
        ...
        -- Trở lại bình thường
        TweenService:Create(TogFrame1, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(1, isHovering and -5 or -10, 0, 0)
        }):Play()
        TweenService:Create(ButtonBG, TweenInfo.new(0.2), {
            BackgroundTransparency = getgenv().UIColor["Background 1 Transparency"]
        }):Play()
    ...
    function buttonFunc:SetDescription(vl)
        if ButtonDescription then
            ButtonDescription.Text = vl
        end
    end
...
function sectionFunction:AddSlider(Setting)
    ...
    if DefaultValue == nil then DefaultValue = minValue end
    if minValue >= maxValue then maxValue = minValue + 1 end  -- ĐẢM BẢO max > min
    ...
    -- SỬA LỖI: THÊM KIỂM TRA NIL VÀ BẢO VỆ CHO CALLBACK
    local callBackAndSetText = function(val)
        local safeVal = tonumber(val)
        if safeVal == nil then
            safeVal = minValue
        end
        Sliderbox_2.Text = tostring(safeVal)
        if Callback then
            pcall(Callback, safeVal)  -- THÊM PCALL ĐỂ BẮT LỖI CALLBACK
        end
    end
    
    -- SỬA LỖI: THÊM KIỂM TRA CHO DEFAULT VALUE
    if DefaultValue then
        local safeDefault = tonumber(DefaultValue) or minValue
        if safeDefault < minValue then
            safeDefault = minValue
        elseif safeDefault > maxValue then
            safeDefault = maxValue
        end
        local barSize = (safeDefault - minValue) / (maxValue - minValue)
        Bar.Size = UDim2.new(barSize, 0, 1, 0)
        callBackAndSetText(safeDefault)
    end
    ...
    -- SỬA LỖI: THÊM KIỂM TRA TRONG RENDERSTEPPED
    RunService.RenderStepped:Connect(function()
        if holdStarted > 0 and (tick() - holdStarted >= holdTime) and not dragging then
            dragging = true
        end
        ...
        if dragging and dragInput then
            local mouseX = dragInput.Position.X - Bar.AbsolutePosition.X
            local clampedX = math.clamp(mouseX, 0, SizeChia)
            local ratio = clampedX / SizeChia
            local value = minValue + (ratio * (maxValue - minValue))
            
            -- SỬA LỖI: THÊM KIỂM TRA PRECISE VÀ ROUNDING
            if Precise or (Setting.Rouding and Setting.Rouding > 0) then
                local decimalPlaces = Setting.Rouding or 1
                value = tonumber(string.format("%." .. decimalPlaces .. "f", value))
            else
                value = math.floor(value)
            end
            
            -- ĐẢM BẢO VALUE KHÔNG VƯỢT QUÁ GIỚI HẠN
    ...
    -- SỬA LỖI: THÊM KIỂM TRA CHO GetSliderValue
    local function GetSliderValue(Value)
        ...
        if numValue <= minValue then
            ...
            callBackAndSetText(minValue)
        elseif numValue >= maxValue then
            Bar.Size = UDim2.new(1, 0, 0, 6)
            callBackAndSetText(maxValue)
        else
            local ratio = (numValue - minValue) / (maxValue - minValue)
            Bar.Size = UDim2.new(ratio, 0, 0, 6)
            callBackAndSetText(numValue)
        end
    end
