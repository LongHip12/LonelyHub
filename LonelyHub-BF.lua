...
if _G.FastAttack then
    ...
    if not Player then
        warn("Khong tim thay nguoi choi cuc bo")
        return
    end
...
function CheckQuest() 
    ...
    elseif World3 then
        ...
        elseif (MyLevel >= 2550 and MyLevel <= 2574)
            Mon = "Serpent Hunter"
            LevelQuest = 1
        ...
        elseif (MyLevel >= 2575 and MyLevel <= 2599)
            Mon = "Skull Slayer"
            LevelQuest = 2
            NameQuest = "TikiQuest3"
        ...
        elseif (MyLevel >= 2600 and MyLevel <= 2624)
            Mon = "Reef Bandit"
            LevelQuest = 1
        ...
        elseif (MyLevel >= 2625 and MyLevel <= 2649)
            Mon = "Coral Pirate"
            LevelQuest = 2
            NameQuest = "SubmergedQuest1"
        ...
        elseif (MyLevel >= 2650 and MyLevel <= 2674)
            Mon = "Sea Chanter"
            LevelQuest = 1
...
print("--[[Loaded UI]]--")
local Fluent = loadstring(game:HttpGet("https://raw.githubusercontent.com/LongHip2012/FluentRemake/refs/heads/main/release.lua.txt"))()
Window = Fluent:CreateWindow({
    Title = "Lonely Hub-Blox Fruit [Free]",
...
local _Toggle = Settings:AddToggle("Toggle", {Title = "Bring Mob V2", Description = "Pls turn off bring v1 after enable to avoid bug!\ni think is better than bring v1", Default = true})
_Toggle:OnChanged(function(Value)
	getgenv().BringMonster = Value
end)

local function _TweenObject(Object, Pos, Speed)
	Speed = Speed or 350
	if not Object or not Pos then return end
	local Distance = (Pos.Position - Object.Position).Magnitude
	local info = TweenInfo.new(Distance / Speed, Enum.EasingStyle.Linear)
	local tween = _TweenService:Create(Object, info, {CFrame = Pos})
	tween:Play()
end

local function _GetMobPosition(MobName)
	local totalPos = Vector3.zero
	local count = 0
	for _, mob in pairs(_Workspace.Enemies:GetChildren()) do
		if mob.Name == MobName and mob:FindFirstChild("HumanoidRootPart") then
			totalPos += mob.HumanoidRootPart.Position
			count += 1
		end
	end
	if count == 0 then
...
local function _BringMob()
	...
	local PlayerHRP = _LocalPlayer.Character.HumanoidRootPart
	local Enemies = _Workspace.Enemies:GetChildren()
	if #Enemies == 0 then return end

	local AveragePositions = {}
	for _, mob in pairs(Enemies) do
		if not AveragePositions[mob.Name] then
			AveragePositions[mob.Name] = _GetMobPosition(mob.Name)
		end
	end
		...
		if mob:FindFirstChild("Humanoid") and mob:FindFirstChild("HumanoidRootPart") then
			...
			if humanoid.Health > 0 then
				...
				if distance <= 350 then
					local mobPos = AveragePositions[mob.Name]
					if mobPos then
						local targetCFrame = CFrame.new(mobPos)
						local distToTarget = (hrp.Position - targetCFrame.Position).Magnitude
						if distToTarget > 3 and distToTarget <= 280 then
							_TweenObject(hrp, targetCFrame, 300)
...
ChooseSkill = Window:AddTab({ Title = "Tab Setting Hold \n and Select Skill", Icon = "" })
...
Volcanic = Window:AddTab({ Title = "Tab Volcano Event", Icon = "" })
Volcanic:AddButton({
...
Get = Window:AddTab({ Title = "Tab Get And Upgrade Items", Icon = "" })
...
Wbk = Window:AddTab({ Title = "Tab Webhook", Icon = "" })
local HttpService = game:GetService("HttpService")
local player = game.Players.LocalPlayer

local webhookUrl = ""
local pingValue = ""
local fruitListText = "Bomb Fruit, Flame Fruit" -- Giả lập trái cây để test
...
Wbk:AddInput("Input", {
    Title = "Ping Everyone/Id",
    Default = "",
    Placeholder = "Everyone Or User ID",
    Numeric = false,
    Finished = false,
    Callback = function(Value)
        pingValue = Value
        print("Ping set to:", pingValue)
    end
})
    ...
    Callback = function()
        if webhookUrl == "" then
            return Window:Dialog({
                Title = "Error",
                Content = "Please enter webhook first",
                Buttons = {{ Title = "OK" }}
            })
        end
        ...
        Window:Dialog({
            Title = "Confirm",
            Content = "Do You Want Send Webhook?",
            Buttons = {
                {
                    Title = "Confirm",
                    Callback = function()
                        -- Ping xử lý
                        local pingContent = ""
                        if pingValue == "" or string.lower(pingValue) == "everyone" then
                            pingContent = "@everyone"
                        else
                            pingContent = "<@" .. pingValue .. ">"
                        end

                        -- Tạo dữ liệu gửi Webhook
                        local data = {
                            ["content"] = pingContent,
                            ["embeds"] = {{
                        ...
                        if request then
                            ...
                            Window:Dialog({
                                Title = success and "Successfuly!" or "Failed!",
                                Content = success and "Send Webhook Successfuly!" or ("Error: " .. tostring(err)),
                                Buttons = {{ Title = "OK" }}
                            })
                        else
                            warn("Không tìm thấy request API.")
                        end
                    end
                },
                {
